...
// @ts-ignore
import ditherFragmentShader from './shaders/largedither.frag'

import * as gfx from 'gophergfx'


export class LargeDitherMaterial extends gfx.Material3
{
    ...
    public thresholdMap: gfx.Texture;
    private thresholdMapUniform: WebGLUniformLocation | null;

    constructor(thresholdMap: gfx.Texture)
    {
        super();
        ...
        LargeDitherMaterial.shader.initialize(this.gl);

        this.kAmbientUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'kAmbient');
        this.kDiffuseUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'kDiffuse');
        this.kSpecularUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'kSpecular');
        this.shininessUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'shininess');

        this.textureUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'textureImage');
        this.useTextureUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'useTexture');

        this.modelUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'modelMatrix');
        this.normalModelUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'normalModelMatrix');
        this.viewUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'viewMatrix');
        this.projectionUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'projectionMatrix');

        this.eyePositionWorldUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'eyePositionWorld');
        this.numLightsUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'numLights');
        this.lightTypesUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'lightTypes');
        this.lightPositionsWorldUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'lightPositionsWorld');
        this.ambientIntensitiesUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'ambientIntensities');
        this.diffuseIntensitiesUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'diffuseIntensities');
        this.specularIntensitiesUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'specularIntensities');

        this.positionAttribute = LargeDitherMaterial.shader.getAttribute(this.gl, 'position');
        this.normalAttribute = LargeDitherMaterial.shader.getAttribute(this.gl, 'normal');
        this.colorAttribute = LargeDitherMaterial.shader.getAttribute(this.gl, 'color');
        this.texCoordAttribute = LargeDitherMaterial.shader.getAttribute(this.gl, 'texCoord');   

        this.thresholdMap = thresholdMap;
        this.thresholdMapUniform = LargeDitherMaterial.shader.getUniform(this.gl, 'thresholdMap');
        this.thresholdMap.setWrapping(false);
    }
    ...
    draw(mesh: gfx.Mesh3, camera: gfx.Camera, lightManager: gfx.LightManager): void
    {
        ...
        this.gl.useProgram(LargeDitherMaterial.shader.getProgram());
        ...
        // Set up Threshold Map
        this.gl.activeTexture(this.gl.TEXTURE0 + this.thresholdMap.id)
        this.gl.bindTexture(this.gl.TEXTURE_2D, this.thresholdMap.texture);
        this.gl.uniform1i(this.thresholdMapUniform, this.thresholdMap.id);
