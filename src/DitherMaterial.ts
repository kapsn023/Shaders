...
export class DitherMaterial extends gfx.Material3
{
    ...
    constructor()
    {
        ...
        DitherMaterial.shader.initialize(this.gl);

        this.kAmbientUniform = DitherMaterial.shader.getUniform(this.gl, 'kAmbient');
        this.kDiffuseUniform = DitherMaterial.shader.getUniform(this.gl, 'kDiffuse');
        this.kSpecularUniform = DitherMaterial.shader.getUniform(this.gl, 'kSpecular');
        this.shininessUniform = DitherMaterial.shader.getUniform(this.gl, 'shininess');

        this.textureUniform = DitherMaterial.shader.getUniform(this.gl, 'textureImage');
        this.useTextureUniform = DitherMaterial.shader.getUniform(this.gl, 'useTexture');

        this.modelUniform = DitherMaterial.shader.getUniform(this.gl, 'modelMatrix');
        this.normalModelUniform = DitherMaterial.shader.getUniform(this.gl, 'normalModelMatrix');
        this.viewUniform = DitherMaterial.shader.getUniform(this.gl, 'viewMatrix');
        this.projectionUniform = DitherMaterial.shader.getUniform(this.gl, 'projectionMatrix');

        this.eyePositionWorldUniform = DitherMaterial.shader.getUniform(this.gl, 'eyePositionWorld');
        this.numLightsUniform = DitherMaterial.shader.getUniform(this.gl, 'numLights');
        this.lightTypesUniform = DitherMaterial.shader.getUniform(this.gl, 'lightTypes');
        this.lightPositionsWorldUniform = DitherMaterial.shader.getUniform(this.gl, 'lightPositionsWorld');
        this.ambientIntensitiesUniform = DitherMaterial.shader.getUniform(this.gl, 'ambientIntensities');
        this.diffuseIntensitiesUniform = DitherMaterial.shader.getUniform(this.gl, 'diffuseIntensities');
        this.specularIntensitiesUniform = DitherMaterial.shader.getUniform(this.gl, 'specularIntensities');

        this.positionAttribute = DitherMaterial.shader.getAttribute(this.gl, 'position');
        this.normalAttribute = DitherMaterial.shader.getAttribute(this.gl, 'normal');
        this.colorAttribute = DitherMaterial.shader.getAttribute(this.gl, 'color');
        this.texCoordAttribute = DitherMaterial.shader.getAttribute(this.gl, 'texCoord');   
    }
    ...
    draw(mesh: gfx.Mesh3, camera: gfx.Camera, lightManager: gfx.LightManager): void
    {
        ...
        this.gl.useProgram(DitherMaterial.shader.getProgram());
