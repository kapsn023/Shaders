...
`)}var R=x(require("fs"),1),b=x(require("util"),1),w=x(require("path"),1),j=b.promisify(R.readFile);async function L(t,e,r){let s=await j(t,"utf8");if(!r)return{contents:s};let o=[],c=[],a=new Set;e.set(t,s);let i=/#include +["']([.\\/\w-]+)["']/g,n=/\r|\n|\r\n/g,l=i.exec(s);for(;l!==null;){let d=l[0],m=l[1],g=w.join(w.dirname(t),m);try{let p=e.get(g);if(p===void 0){let u=await L(g,e,r);u.warnings?.forEach(f=>c.push(f)),u.watchFiles?.forEach(f=>a.add(f)),p=u.contents,e.set(g,p)}o.push({file:g,contents:p,target:d}),a.add(g),l=i.exec(s)}catch{if(l===null)break;let u=s.split(n),f=u.indexOf(l[0]),y=u[f];c.push({text:`File from <${l[0]}> not found`,location:{file:m,line:f+1,length:m.length,column:y.indexOf(m),lineText:y}}),o.push({file:g,contents:"",target:l[0]}),l=i.exec(s)}}for(let d of o)s=s.replace(d.target,d.contents);return{contents:s,warnings:c,watchFiles:[...a]}}function k({minify:t=!1,resolveIncludes:e=!0}={}){let r=new Map;return{name:"glsl",setup(s){async function o(c){let{contents:a,warnings:i,watchFiles:n}=await L(c.path,r,e);return{contents:t?S(a):a,warnings:i,watchFiles:n,loader:"text"}}s.onLoad({filter:/\.(?:frag|vert|glsl|wgsl)$/},o)}}}0&&(module.exports={glsl});
